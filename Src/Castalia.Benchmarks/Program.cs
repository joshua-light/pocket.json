...
namespace Castalia.Benchmarks
{
    public class Program
    {
        ...
        private static void Serialize<T>() where T : new()
        {
            ...
            Run("Nothing", 100000, () => item.AsJson());
        ...
        private static void Deserialize<T>(T item)
        {
            ...
            Run("Nothing", 100000, () => json.AsJson<T>());
        ...
        private static void Run(string name, int times, Action action)
        {
            ...
            sw.Stop();
            var allocated = (double) (GC.GetTotalMemory(false) - memory) / 1000 / 1000;
            collections = GC.CollectionCount(0) + GC.CollectionCount(1) + GC.CollectionCount(2) - collections;

            Console.WriteLine(name + "[" + times + "]: " + sw.ElapsedMilliseconds + " ms. Allocated: " +
                allocated.ToString("N") + " MB. Collections: " + collections + ".");
